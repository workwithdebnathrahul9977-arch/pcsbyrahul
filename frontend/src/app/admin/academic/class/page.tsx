...
export default function ClassManagementPage() {
  ...
  const fetch = async () => {
    const res = await axios.get(`${API}/api/academic/classes`);
    setItems(res.data);
  };
  useEffect(() => { fetch(); }, []);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    await axios.post(`${API}/api/academic/classes`, { name });
  ...
  const handleEdit = async (item: any) => {
    ...
    if (newName && newName !== item.name) {
      await axios.put(`${API}/api/academic/classes/${item.id}`, { name: newName });
  ...
  const handleDelete = async (id: string) => {
    ...
    await axios.delete(`${API}/api/academic/classes/${id}`);
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 min-h-[80vh]">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">Manage academic classes (e.g. Class 9, 10, SSC)</p>
        </div>
      </div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-6">
        ...
        <form onSubmit={handleCreate} className="flex gap-3">
          <input
            type="text"
            placeholder="e.g. Class 9, Class 10, SSC"
            value={name}
      ...
      <div className="overflow-x-auto border border-gray-100 rounded-xl">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={3} className="p-8 text-center text-gray-400">No classes found. Add one above.</td></tr>
            ) : items.map((item, i) => (
